...
def export_job(job_id):
    ...
    for r in job["results"]:
        if r.get("passed") and r.get("frameId"):
...
def build_playback(job_id):
    """
    Rebuild the source clip at original fps/timing, but with every rejected
    frame (blur or low similarity) replaced by a black frame labeled with
    the reject reason — so you can scrub/play and see exactly where drift
    or blur happens in real time, not just as a static ranked list.
    """
    import cv2
    ...
    # index results by frame number for O(1) lookup while writing
    ...
    # Write raw frames with mp4v first (widely supported by OpenCV's writer),
    # then transcode to H.264/yuv420p via ffmpeg. Browsers (Safari & Chrome)
    # will not decode mp4v inline in a <video> tag -- it "plays" silently
    # with nothing rendered, which is what was happening before this fix.
    ...
    if ffmpeg_bin:
        ...
        # No ffmpeg on PATH -- fall back to the raw file. It will download
        # fine but likely won't play inline in the browser.
        out_path = raw_path
...
def preview_size(asset_id):
    """Larger image for the hover-preview panel. Immich's own 'preview' size
    (not 'thumbnail') -- big enough to actually judge blur/pose by eye."""
    r = requests.get(
        f"{IMMICH_BASE_URL}/api/assets/{asset_id}/thumbnail",
        headers={"x-api-key": IMMICH_API_KEY},
        params={"size": "preview"},
...
function previewUrlFor(r) {
  // Video-analysis frames (r.thumbUrl set) are already full-size on disk --
  // reuse that. Immich assets get the bigger 'preview' size instead of the
  // small thumbnail used in the ring/list.
...
function showHoverPreview(r) {
  ...
  // Tiny delay avoids flashing full-res loads while quickly sweeping the
  // mouse across many bubbles/rows.
...
function render(centerId, data, centerThumbUrl) {
  ...
  bands.forEach((bandResults, bandIdx) => {
    ...
    bandResults.forEach((r, i) => {
      ...
      node.onclick = () => r.assetId ? loadNeighbors(r.assetId) : null;
  ...
  results.forEach(r => {
    ...
    `;
    row.onclick = () => r.assetId ? loadNeighbors(r.assetId) : null;
...
function applyFisheye(mx, my) {
  ...
  document.querySelectorAll('.node').forEach(node => {
    ...
    if (dist < FISHEYE_RADIUS) {
      ...
      // Push direction is radial from the ring's CENTER, not from the
      // cursor. Pushing away from the cursor made nodes visibly dodge the
      // pointer as you approached them (direction flips as the mouse
      // angle relative to the node changes) -- impossible to click.
      // Pushing along a fixed center->node direction means a node only
      // ever grows/recedes along one line, so it holds still under the
      // cursor once you're over it.
...
// ---- video-clip frame scrubbing & preview ----
...
document.addEventListener('keydown', (e) => {
  ...
  if (document.activeElement &&
      ['INPUT', 'TEXTAREA', 'BUTTON'].includes(document.activeElement.tagName)) return;
...
async function pollAnalysis(jobId, videoFileName, refFrameIdx) {
  ...
  render('__anchor__', { mode: 'face', results: [{ assetId: '__anchor__', filename: `Frame ${refFrameIdx} (Anchor)`, similarity: 1.0, thumbUrl: anchorUrl }, ...results] }, anchorUrl);
}
</script>
</body>
</html>
