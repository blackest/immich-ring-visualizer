...
<div id="left-panel">
  ...
  <div id="left-panel-content">
    ...
    <div class="panel-section expanded" data-section="video-analysis">
      <div class="panel-section-header"><span class="chev">▸</span><span class="sec-title">Video Analysis</span></div>
      <div class="panel-section-body">
...
async function pollAnalysis(jobId, videoFileName, refFrameIdx) {
  ...
  videoStatus.innerHTML = `
    ...
    <div style="margin-top:12px;padding-top:10px;border-top:1px solid #22222a;">
      ...
      <label style="display:flex;align-items:center;gap:6px;font-size:10px;color:var(--dim);margin-bottom:6px;">
        <input id="crosscheck-show-list" type="checkbox" checked>
        Show closest match list
      </label>
      <div style="display:flex;gap:6px;">
  ...
  const ccShowList = document.getElementById('crosscheck-show-list');
  if (ccShowList) {
    ccShowList.checked = localStorage.getItem('ringviz.crosscheckShowList') !== '0';
    ccShowList.addEventListener('change', () => {
      localStorage.setItem('ringviz.crosscheckShowList', ccShowList.checked ? '1' : '0');
      const out = document.getElementById('crosscheck-results');
      if (out && !ccShowList.checked) out.innerHTML = '';
    });
  }
  document.getElementById('crosscheck-btn').onclick = async () => {
    ...
    try {
      ...
      } else if (ccShowList && !ccShowList.checked) {
        out.innerHTML = `<div style="font-size:11px;color:var(--dim);">Found ${result.results.length} Immich matches. Enable the list to add individual matches to the ring.</div>`;
      } else {
  ...
  if (curModeEl) curModeEl.textContent = 'VIDEO FRAME ANALYSIS (local, not in Immich)';
