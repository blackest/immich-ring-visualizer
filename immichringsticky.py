...
def run_video_analysis(job_id, video_path, anchor_path, sim_threshold, blur_threshold, ref_frame_idx=1):
    ...
    try:
        ...
        while cap.isOpened():
            ...
            if not faces:
                results.append({"frame": frame_idx, "sim": 0.0, "blur": 0.0, "passed": False, "hasFace": False})
                job["results"] = results
                continue
            ...
            if passed:
                ...
                # Keep the raw embedding server-side only, keyed by frame
                # number, so a later cross-check against Immich's own
                # face_search table can query it directly. This is
                # deliberately NOT put on the result dict below --
                # job["results"] is re-serialized to JSON on every status
                # poll, and 512 floats x however many frames would bloat
                # that payload for no benefit to the browser.
...
def export_job(job_id):
    ...
    # Optional selection: { "frames": [12, 45, 88] } in the POST body.
    # When present, only those frame numbers get exported instead of
    # every frame that passed the sim/blur gates -- lets someone hand-pick
    # a smaller, curated set from the ranked sidebar rather than always
    # dumping the whole passed set to disk.
...
def immich_cross_check(job_id, frame_no):
    """Compare one already-analyzed video frame against everything already
    stored in the user's Immich library, using the SAME face_search table
    and pgvector distance the normal ring view queries -- just starting
    from a raw embedding computed live from the clip, instead of an
    assetId that already exists in the database. Answers 'is this new
    generated frame actually the same person as what's already in Immich',
    not just 'is it consistent with this clip's own anchor frame'."""
    job = _analysis_jobs.get(job_id)
    if not job:
        return jsonify({"error": "unknown job"}), 404
    ...
    # pgvector wants a literal like '[0.1,0.2,...]' cast to ::vector --
    # psycopg2 doesn't know that type natively, so build the literal
    # string ourselves rather than requiring the pgvector python package
    # be installed just for this one query.
...
def export_immich_assets():
    """Save selected Immich library images to disk at full resolution --
    works standing alone, with no video analysis involved at all. Fetches
    each asset's ORIGINAL file from Immich's own API (not the thumbnail
    used for on-screen display), so what lands on disk is the real
    source image, not a downsized preview."""
    body = request.get_json(silent=True) or {}
...
function showHoverPreview(r) {
  ...
  hoverTimer = setTimeout(() => {
    ...
    hoverCaption.textContent = `${r.filename} — ${pct}%`;
...
function render(centerId, data, centerThumbUrl) {
  ...
  bands.forEach((bandResults, bandIdx) => {
    ...
    const baseRadius = radiusForSim(avgSim);
    const size = sizeForSim(avgSim);

    const minCircumference = bandResults.length * (size + 14);
    const neededRadius = minCircumference / (2 * Math.PI);
    ...
    bandResults.forEach((r, i) => {
      ...
      // Nodes pulled in via "Add to ring" from an Immich cross-check are
      // marked distinctly (amber ring) so they're never mistaken for a
      // frame that actually came from this clip.
      if (r.fromImmich) {
      ...
      // Clicking a normal neighbor recenters browsing on it -- but doing
      // that for an injected Immich node would abandon the whole video
      // ring it was added to compare against, which defeats the point.
  ...
  results.forEach(r => {
    ...
    const pct = (r.similarity * 100).toFixed(1);
    // Video-analysis frames (from the video pipeline, not real Immich
    // assets) carry a raw `frame` number -- those get the frame-export
    // checkbox. Anything with a real assetId (pure Immich browsing rows,
    // or Immich nodes pulled into a video ring via "Add to ring") gets
    // the separate, persistent Immich-selection checkbox instead.
    let checkboxHtml = '';
    ...
    `;
    row.onclick = (e) => {
      // A click on either checkbox should only toggle selection, not
      // also recenter the ring. Immich-sourced rows also don't recenter
      // on click when they're an injected ring node -- doing so would
      // abandon the video-analysis ring they were added to compare
      // against. Plain Immich-browsing rows (not fromImmich-tagged)
      // still recenter as before.
...
// ---- selection mode for "save selected frames to disk" ----
// A Set of raw frame numbers, independent of the Immich-neighbors mode
// (where selection/export doesn't apply). Persists across re-renders of
// the same job so toggling a checkbox, scrolling, and re-rendering the
// ring doesn't silently lose what was picked.
...
// ---- selection mode for Immich library images ("export selected + related") ----
// Deliberately separate from selectedFrames above, and deliberately
// NEVER auto-cleared by render(), by recentering the ring, by starting a
// new video analysis, or by anything else -- this is the "sticky"
// selection: check something while browsing normally, click around to
// look at a few other clusters, and what you checked earlier is still
// checked when you scroll back to it. Works with zero video analysis
...
document.getElementById('immich-clear-selected-btn').addEventListener('click', () => {
  ...
  updateImmichSelectionBar();
  // Re-check whichever mode is currently active so unchecked boxes
  // reflect immediately rather than waiting for the next navigation.
...
async function pollAnalysis(jobId, videoFileName, refFrameIdx) {
  ...
  // Kept so "Add to ring" (from an Immich cross-check match) can
  // re-render this same view later with the extra node merged in,
  // without redoing the whole poll/analysis pipeline.
  lastVideoRingState = {
    anchorUrl,
    refFrameIdx,
    baseResults: results,
...
// ---- pulling Immich matches into the same ring/sidebar as video frames ----
// Cross-check results normally sit in their own small box, disconnected
// from the main ring. "Add to ring" lets a specific match be pulled in
// as an extra node alongside the video frames, so you can visually
// compare a generated frame against something already in the library in
// the same view instead of two separate places on the page.
...
function addImmichNodeToRing(match) {
  if (extraImmichNodes.some(n => n.assetId === match.assetId)) return; // no duplicates
