"""NG twin of image_ops.py -- crop/resize/export-sizing helpers used by
the Export Settings panel.

NG-only file: per the NG duplication rule in APP_ARCHITECTURE_NOTES.md,
this does not import from or call into image_ops.py. Pure functions,
no module-level state, so unlike detectionNG.py/stateNG.py there's no
per-project isolation concern here -- it's duplicated purely so NG never
depends on the original module.
"""

import numpy as np


def _pick_interp_ng(name, scale):
    import cv2
    table = {
...
def _center_crop_to_aspect_ng(img, out_w, out_h):
    h, w = img.shape[:2]
...
def crop_resize_export_ng(img, bbox, out_w, out_h, mode="face", margin=2.2, interp="auto", upscale=True, max_upscale=None, pad_mode="none", native=False):
    ...
    if mode == "face" and bbox:
        ...
        crop = _center_crop_to_aspect_ng(img, out_w, out_h)
...
def _as_bool_ng(value, default=False):
...
def _export_params_from_body_ng(body):
    ...
    return {
        ...
        "upscale": _as_bool_ng(body.get("upscale"), True),
        "max_upscale": max_upscale,
        "pad_mode": body.get("padMode") or "none",
        "min_face_px": min_face_px,
        "native": _as_bool_ng(body.get("native"), False),
    }


def face_export_height_px_ng(img, bbox, p):
    if not bbox:
        return None
...
def should_skip_for_small_face_ng(img, bbox, p):
    if p["min_face_px"] <= 0:
        return False, None
    height_px = face_export_height_px_ng(img, bbox, p)
